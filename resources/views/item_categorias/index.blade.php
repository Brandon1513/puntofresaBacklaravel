        ...
        <div class="max-w-5xl mx-auto space-y-4 sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden bg-white rounded shadow-sm">
                <div class="flex items-center justify-between px-4 py-3 border-b">
                    ...
                    <a href="{{ route('item-categorias.create') }}"
                    ...
                    <table class="w-full text-sm border-collapse">
                        ...
                        @forelse($categorias as $c)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2 border">{{ $c->nombre }}</td>
                                <td class="px-3 py-2 border">{{ $c->descripcion ?? '—' }}</td>
                                ...
                                <td class="px-3 py-2 border">
                                    <div class="flex gap-2">
                                        <a href="{{ route('item-categorias.edit', $c) }}"
                                           class="text-xs text-indigo-600 hover:underline">
                                            Editar
                                        </a>

                                        <form method="POST"
                                              action="{{ route('item-categorias.toggle', $c) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
